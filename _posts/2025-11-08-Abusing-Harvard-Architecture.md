...
1. `A` - Load/Pointer register:
    1. Can load any 15-bit integer value.
...
1. Load Instruction - Sets `A` to a 15-bit integer. Its assembly representation is `@<integer>`, e.g., `@7447` or `@32223`.
...
0   @0                  // Load 0 into A
1   D=M                 // Load the value in the RAM in the address A into D
2   @ALREADY_POSITIVE   // Load the ROM address of (ALREADY_POSITIVE) into A
3   D;JGE               // if D >= 0 then jump to the address in A
4   @0                  // Load 0 into A
5   M=-M                // Negate M
    (ALREADY_POSITIVE)  // Not an instruction, just a label referring to the
                        // index of the next instruction in the ROM - 6
6   <Whatever is after> // Rest of the ROM
```
...
0   @3                  // Loads 3 into A, equivalent to
                        // @ALREADY_POSITIVE
1   M;JGE               // if M >= 0 then jump to the address in A
2   M=-M                // Negate M
    (ALREADY_POSITIVE)  // Not an instruction, just a label referring to the
                        // index of the next instruction in the ROM, in this
                        // case 3
3   <Whatever is after> // Rest of the ROM
```
...
which has the identical result - advancing `result` and `previous` by one element.
...
We can't move `(LOOP)` to address 0, since that's the start of the program, so let's move `n` to the RAM address equal to the ROM address of `LOOP`, which we will refer to as `*LOOP`. We can do this by simply copying `n` into that address before the loop starts:
...
The 2 instructions we added before the loop are inconsequential compared to removing 2 instructions from every iteration of the loop.

While a 20% improvement is not insignificant in itself, in addition to the immediate benefit, a new opportunity for optimisation just opened up.
